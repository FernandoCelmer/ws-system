...
if($_SESSION['system_user_level'] == 1){
	...
	$sql_res=mysql_query("select * from support_ticket where id_system_user= '".$_SESSION['id_system_user']."'"); //consulta no banco
  ...
  <div class="w3-row-padding w3-margin-bottom">
		<table class="w3-table w3-small w3-bordered w3-responsive w3-border">
			<tr>
				...
				<td width="175"><b>DateTime</b></td>
				<td><b>Assunto</b></td>			
				<td align='right' width="25"><b></b></td>
			</tr>
			<?php
				$sql="select * from support_ticket where id_system_user= '".$_SESSION['id_system_user']."' ORDER BY id_support_ticket DESC LIMIT $inicio, $maximo"; //consulta no BD
				...
				$id = $res[id_support_ticket];
				$subject = $res[support_ticket_subject];
				$status = $res[support_ticket_status];
				$date = $res[support_ticket_datetime_open];
			?>
			<tr>
				<td><?php if($status == "TRUE"){ echo "<span class='w3-tag w3-light-green w3-round'><i><i class='fa fa-check'></i>Aberto</i></span>";}else echo "<span class='w3-tag w3-light-grey w3-round'><i><i class='fa fa-check'></i>Fechado</i></span>";?></td></td>
